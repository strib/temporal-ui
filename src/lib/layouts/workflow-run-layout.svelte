<script lang="ts">
  ...
  import {
    ...
    shouldTriggerWorkflowCompletionCelebration,
  ...
  $: {
    ...
    if (!wf) {
      ...
      const current = wf.status;
      if (
        shouldTriggerWorkflowCompletionCelebration(previousWorkflowStatus, current)
      ) {
        blastWorkflowCompletionConfetti();
      }
      previousWorkflowStatus = current;
    }
  }
